...
process_execute (const char *file_name) 
{
  ...
  struct parentChild *pC = (struct parentChild*) malloc(sizeof(struct parentChild));
  pC->stillAlive = 2;
  pC->hasWaited = false;
  struct thread *parentThread = thread_current();
  pC->parent = parentThread;
  sema_init(&(pC->sema), 0);
  sema_init(&(pC->waitSema), 0);
  lock_init(&(pC->exitLock)); //New Lock added
  ...
  tid = thread_create(name, PRI_DEFAULT, start_process, pC);
  sema_down(&(pC->sema));
...
{
  struct parentChild *pC = (struct parentChild*)aux;
  ...
  success = load (pC->fileName, &if_.eip, &if_.esp);
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  if(list_empty(&(t->children)) || child_tid == TID_ERROR){
    return -1;
  }
    ...
    el = list_next(el))
  {
    ...
    tid_t tid = pC->tid;
    if(tid == child_tid && !pC->hasWaited){
      pC->hasWaited = true;
      exitStatus = pC->exit_status;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  for(token = strtok_r(file_name, " ", &save_ptr); token != NULL; 
    ...
    *esp -= tokenSize;
    argv[argc] = memcpy(*esp, token, tokenSize);
    argc++;
  }

  //Word alignment
  *esp -= (int32_t)(*esp)%sizeof(void*);
  *esp -= sizeof(char*);
  ...
  //Write pointer argv to stack
  void* argvPtr = *esp;
  *esp -= sizeof(char**);
  memcpy(*esp, &argvPtr, sizeof(char**));
